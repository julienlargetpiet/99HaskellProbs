...
getRangeList :: ByteString -> [Int] -> ByteString
getRangeList _ [] = B.empty
getRangeList bs (idx:ids) = 
    B.cons (B.index bs idx) (getRangeList bs ids) 
...
subCalc xs ids nums =
    let curmax = myMax nums
        ...
        xsstop =
            if idstop + 1 < B.length xs
            then getRangeList xs [(idstop + 1)..(B.length xs - 1)]
            else B.empty

        xsbetween =
            getRangeList xs [(idstrt + 1)..(idstop - 1)]
